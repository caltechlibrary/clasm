...
func CreateInstanceFromAMI(ctx context.Context, w io.Writer, ec2Clients map[string]awsclient.EC2API, ssmClients map[string]awsclient.SSMAPI, iamClient awsclient.IAMAPI, images []inventory.Image) error {
	...
	return createInstanceFromAMI(ctx, w, ec2Clients, ssmClients, iamClient, image, nil, nil)
}
...
func createInstanceFromAMI(ctx context.Context, w io.Writer, ec2Clients map[string]awsclient.EC2API, ssmClients map[string]awsclient.SSMAPI, iamClient awsclient.IAMAPI, image inventory.Image, menuInput io.Reader, menuOutput io.Writer) error {
	params, ec2Client, ssmClient, err := collectLaunchInstanceParams(ctx, w, ec2Clients, ssmClients, iamClient, image, menuInput, menuOutput)
	if err != nil {
		return cancelledIsNil(w, err)
	}
